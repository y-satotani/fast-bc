...
\begin{frame}{概要}
  ...
  \end{itemize}
  \note[item]{
    ネットワークの中の重要なノードを見つけることはネットワーク解析において重要な課題です．
  }
  \note[item]{ノードの重要さを測る指標として媒介中心性がよく用いられます．}
  \note[item]{媒介中心性の値を高速に計算する方法としてBrandesのアルゴリズムが知られてます．}
  \note[item]{
    しかし現実のネットワークはノードやリンクの挿入や削除によって絶えずその形を変化させ続けています．
    そのようなネットワークの媒介中心性を求める場合，変化の影響を受ける部分だけを更新してさらなる高速化が期待できます．
  }
  \note[item]{
    そこで，この研究では，最短経路の更新を利用して媒介中心性を更新するアルゴリズムを提案し，評価します．
  }
  \note[item]{
...
\begin{frame}[t]{研究背景 -- 媒介中心性}
  ...
  \note[item]{以降，各要素について詳しくのべます．まず，研究背景として媒介中心性について説明します．}
  \note[item]{
  ...
  \note[item]{
    定義通りに媒介中心性を計算するアルゴリズムは，
    総和記号のこの部分から，和は頂点数$|V|$の二乗だけ繰り返されます．
...
\begin{frame}{研究目的}
  ...
  \note[item]{
    専用のデータ構造を用いた手法や，時不変ネットワークで用いられた近似法を応用した手法の他に，
    最短経路を保持し，媒介中心性と共に更新する方法も提案されてきました．
    このような方法は，最短経路を更新するアルゴリズムに基づいています．
  }
  \note[item]{
    こちらに，最短経路を更新する代表的なアルゴリズムを示しました．
  }
  \note[item]{
    Demetrescu-Italiano法に基づく媒介中心性更新法がNasreらによって提案されましたが，
    必要とする最悪空間計算量は頂点数の$3$乗のオーダで，大規模なネットワークの解析には不向きです．
  }
  \note[item]{
    しかし，Ramalingam-Reps法の最悪空間計算量は頂点数の$2$乗オーダで，
    この方法を基にする媒介中心性更新法の空間計算量は比較的小さいことが期待できます．
  }
  \note[item]{
...
\begin{frame}[allowframebreaks]{提案アルゴリズム}
  ...
  \begin{itemize}\small
    ...
    \item $\Delta_{s\bullet}(x)=\sum_{t\in T(s)}\delta_{st}(x),\qquad\Delta'_{s\bullet}(x)=\sum_{t\in T(s)}\delta'_{st}(x)$：
    \item[] 削除の影響を受けるペア依存度の総和\footcite{31Bergamini2017}
    \item $\tau(s)=T(s)\cup\{t|\Delta_{s\bullet}(t)>0\},\qquad\tau'(s)=T(s)\cup\{t|\Delta'_{s\bullet}(t)>0\}$：
...
\begin{frame}{影響を受ける頂点}
  ...
  \note[item]{
    読み上げますと，頂点$s,t$に対して，最短経路が変化するための
    必要十分条件はこちらの式を満たすことです．
  }
  \note[item]{
