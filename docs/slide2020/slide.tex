...
\begin{frame}[allowframebreaks]{提案アルゴリズム}
  \alert{TODO} 整形
  \begin{itemize}\small
    \item 辺$(u,v)$削除時の媒介中心性の値を更新するアルゴリズムを提案する．
    \item 削除の操作後であることを明示するため，記号$'$をつける．
    \item 頂点$t$に対して，辺操作によって$t$への最短経路が変化する頂点の集合を$S(t)$と表す．
    \item 同様に，頂点$s$に対して，辺操作によって$s$からの最短経路が変化する頂点の集合を$T(s)$と表す．
    \item 影響を受ける依存度の総和$\Delta_{s\bullet}(x)=\sum_{t\in T(s)}\delta_{st}(x)$\cite{31Bergamini2017}．
  \end{itemize}
...
\begin{frame}{影響を受ける頂点}
  ...
  補題から$T(s)$と$S(x)$の具体的な内容がわかる
  \begin{itemize}
  \item $T(x)=\{t|t\in V\land d_{xt}=d_{xu}+l_{uv}+d_{vt}\}$
  \item $S(x)=\{s|s\in V\land d_{sx}=d_{su}+l_{uv}+d_{vx}\}$
  \end{itemize}
\end{frame}
...
\begin{frame}{$\Delta_{s\bullet}(x)$を求めるアルゴリズム}
  ...
  $\Delta'_{s\bullet}(x)$も同様
\end{frame}

\begin{frame}{辺削除時の最短経路更新法(Ramalingam and Reps)}
  \alert{TODO}
  \begin{itemize}
  \item $\sigma'_{st}$は距離$d'_{st}$の昇順に計算する必要がある．
