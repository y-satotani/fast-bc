...
本章では，後の章で提案するアルゴリズムを説明するための準備をする．はじめに，最短経路や距離を含む，
グラフ理論の基本事項を説明する．
...
頂点および辺は，ネットワークにおけるノードとリンクに対応する．
...
重みなしグラフに対して後述する距離などの重みが必要な量を計算する際，
全ての辺に対して同一の重みが付与されていると仮定する．
特に断らない限り，その重みを$1$とする．

無向グラフにおいて，頂点$v$と辺$e$が\textbf{接続する}とは，$v\in e$つまり，
$e$の端点に$v$が属することをいう．
頂点$v$と頂点$w$が\textbf{隣接する}とは，$(v,w)$ないし$\{v,w\}$が辺集合$E$に属することをいう．
無向グラフ$G$の頂点$v$について，$v$と隣接する頂点の集合を頂点$v$の\textbf{近傍}と
呼び，$\mathcal{N}_G(v)$ないし$\mathcal{N}(v)$で表す．また，無向グラフ$G$について$v$の近傍の数
...
先行と後継を記すとき，近傍と同様にグラフが省略されることがある．つまり，$v$の先行および後継を
$\mathcal{P}(v)$および$\mathcal{S}(v)$と記すことがある．
...
経路グラフに対して，その重みの総和を\textbf{長さ}と呼ぶ．

グラフ$G$上の頂点$s$と頂点$t$について，それらを結ぶ道$P\subset G$のうち，
