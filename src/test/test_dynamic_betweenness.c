...
int test_repeat_directed(unsigned long int seed, int steps);
int test_inc_unweighted(unsigned long int seed);
int test_dec_unweighted(unsigned long int seed);
int test_inc_unweighted_directed(unsigned long int seed);
int test_dec_unweighted_directed(unsigned long int seed);
...
void _incremental_update_unweighted(igraph_t* G,
                                    igraph_matrix_t* D,
                                    igraph_matrix_int_t *S,
                                    igraph_vector_t* B,
                                    igraph_integer_t u,
...
void _decremental_update_unweighted(igraph_t* G,
                                    igraph_matrix_t* D,
                                    igraph_matrix_int_t *S,
                                    igraph_vector_t* B,
                                    igraph_integer_t u,
...
int test_inc_random(unsigned long int seed) {
  ...
  igraph_erdos_renyi_game(&G, IGRAPH_ERDOS_RENYI_GNM, 10, 15, 0, 0);
...
int test_repeat_directed(unsigned long int seed, int steps) {
  ...
  // initialize a graph
  igraph_rng_seed(igraph_rng_default(), seed);
  igraph_erdos_renyi_game(&G, IGRAPH_ERDOS_RENYI_GNM, 20, 50, 1, 0);
  // set weights
  igraph_vector_init(&weights, igraph_ecount(&G));
  for(igraph_integer_t eid = 0; eid < igraph_ecount(&G); eid++) {
    weight = igraph_rng_get_integer(igraph_rng_default(), 1, 5);
  ...
  for(int step = 0; step < steps; step++) {
    ...
    sprintf(test_name, "test_repeat_directed (%lu) at inc step %d", seed, step);
    res |= _check_quantities(test_name, &G, &D, &S, &B, &weights, 0);
    ...
    sprintf(test_name, "test_repeat_directed (%lu) at dec step %d", seed, step);
    int res = _check_quantities(test_name, &G, &D, &S, &B, &weights, 0);
  }
...
int test_inc_unweighted(unsigned long int seed) {
  ...
  _incremental_update_unweighted(&G, &D, &S, &B, u, v);
  igraph_vector_push_back(&weights, weight);
  char test_name[1024];
  sprintf(test_name, "test_inc_unweighted (%lu)", seed);
  int res = _check_quantities(test_name, &G, &D, &S, &B, &weights, 0);
...
int test_dec_unweighted(unsigned long int seed) {
  ...
  char test_name[1024];
  sprintf(test_name, "test_dec_unweighted (%lu)", seed);
  int res = _check_quantities(test_name, &G, &D, &S, &B, &weights, 0);
