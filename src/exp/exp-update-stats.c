...
int main(int argc, char* argv[]) {
  ...
  igraph_vector_copy(&B_old, &B);
  dybc_update_stats_t stats;
  if(arguments.query == QUERY_INSERT)
    incremental_update(&G, &D, &S, &B, u, v, weights, w, &stats);
  else if(arguments.query == QUERY_DELETE)
    decremental_update(&G, &D, &S, &B, u, v, weights, w, &stats);
  else
    assert(0 && "query must be insert or delete");

  long int n_tau_star = 0;
  for(long int i = 0; i < igraph_vcount(&G); i++)
    if(igraph_cmp_epsilon(VECTOR(B)[i], VECTOR(B_old)[i], IGRAPH_SHORTEST_PATH_EPSILON))
      n_tau_star++;
